...
Name: Felix Liu, Brian Truong
Login: cs61a-as, cs61a-yv
TA: 
Section:
"""
...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant != None and insect.can_contain(self.ant):
                insect.contain_ant(self.ant)
                self.ant = insect
            elif self.ant != None and self.ant.can_contain(insect):
    ...
    def remove_insect(self, insect):
        ...
        if type(insect) != QueenAnt or insect.queen_counter > 1:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return self.place.ant is not None and self.place.ant.blocks_path #A7
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        return self.container and not ant.container and not self.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        "*** YOUR CODE HERE ***"
        counter = 0
        n = self.place.entrance
        if self.place.bees == []:
            while n != hive:
                if n.bees != []: #QB4
                    if counter >= self.min_range and counter <= self.max_range:
                        return random_or_none(n.bees)
                n = n.entrance
                counter += 1
        if self.min_range != 0:
            return None  
...
class QueenPlace(object):
    """The place that the queen resides in"""
    def __init__(self,queen_place,colony):
        self.original_queen = colony.queen
    ...
    @property
    def bees(self):
        return self.original_queen.bees + self.queen_place.bees
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        Place.add_insect(self, insect) #Q4
        if insect.watersafe == False: #Q4
            insect.reduce_armor(insect.armor) #Q4 redundant because "invoking"
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in a.bees[:]: #iterate over a copy of list self.place
...
class NinjaAnt(Ant):
    ...
    damage = 1
    implemented = True 
    blocks_path = False #A7 

    def action(self, colony):

        for item in self.place.bees[:]: 
            item.reduce_armor(self.damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.assign = False
        self.queen_counter = QueenAnt.queen_counter

    def action(self, colony):
        """A queen ant throws a leaf, but also doubles the damange of ants
        behind her.  Impostor queens do only one thing: die."""
        if self.queen_counter > 1:
            self.reduce_armor(self.armor)
            QueenAnt.queen_counter -= 1
            return
        if self.assign == False:
            colony.queen = QueenPlace(self.place, colony)
            QueenAnt.assign = True
        ThrowerAnt.action(self, colony)
        if self.place:
            location = self.place.exit
            while location:
                if location.ant and not location.ant.buffed:
                    location.ant.damage *= 2
                    location.ant.buffed = True
                    if location.ant.container:
                        location.ant.ant.damage *= 2
                        location.ant.ant.buffed = True
